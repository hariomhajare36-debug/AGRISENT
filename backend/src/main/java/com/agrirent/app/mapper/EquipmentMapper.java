...
public class EquipmentMapper {
    ...
    public EquipmentResponse toResponse(Equipment equipment) {
        ...
        response.setBrand(equipment.getBrand() != null ? equipment.getBrand() : equipment.getMake());
        response.setModel(equipment.getModel());
        response.setVariant(equipment.getVariant());
        response.setCurrency(equipment.getCurrency() != null ? equipment.getCurrency() : "INR");
        response.setPriceType(equipment.getPriceType() != null ? equipment.getPriceType() : "FIXED");
        response.setPriceSourceName(equipment.getPriceSourceName());
        response.setPriceSourceUrl(equipment.getPriceSourceUrl());
        response.setLastVerifiedDate(equipment.getLastVerifiedDate());
        response.setDistrict(equipment.getDistrict() != null ? equipment.getDistrict() : equipment.getCity());
        response.setLiftingCapacityKg(equipment.getLiftingCapacityKg());
        response.setFuelTankLitres(equipment.getFuelTankLitres());
        response.setEngineCc(equipment.getEngineCc());
        response.setPtoRpm(equipment.getPtoRpm() != null ? equipment.getPtoRpm() : equipment.getPtoSpeed());
